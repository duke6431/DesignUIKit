...
extension PanModal {
    class InteractionController: UIPercentDrivenInteractiveTransition {
        ...
        func wireToViewController(viewController: UIViewController, interactive: Bool = true) {
            ...
            if interactive {
                updateGesture(for: viewController.view, type: UIPanGestureRecognizer.self, target: self, action: #selector(handlePanGestureRecognizer))
            }
        }
        
        func wireToView(view: UIView, interactive: Bool = true) {
            if interactive {
                updateGesture(for: view, type: UIPanGestureRecognizer.self, target: self, action: #selector(handlePanGestureRecognizer))
                updateGesture(for: view, type: UITapGestureRecognizer.self, target: self, action: #selector(handleTapGestureRecognizer))
            }
        }
