...
extension PanModal {
    class AnimationController: NSObject, UIViewControllerAnimatedTransitioning {
        ...
        func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
            ...
            UIView.animateKeyframes(withDuration: transitionDuration, delay: 0, options: .calculationModeCubic, animations: { [weak self] in
        ...
        func frame(presenting: Bool, for direction: OriginDirection, of targetView: UIView, in containerView: UIView) -> CGRect {
            switch direction {
            case .bottom:
                return .init(
                    x: 0,
                    y: presenting ? containerView.bounds.height : containerView.bounds.height - targetView.bounds.height,
