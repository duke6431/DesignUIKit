...
public extension FContaining where Self: UIView {
    ...
    @FViewBuilder
    func `if`<Content: FBodyComponent>(_ condition: @autoclosure () -> Bool, transform: (Self) -> Content) -> FBody {
        if condition() {
            transform(self)
        } else {
            self
    ...
    /// - Returns: Either the original `View` or the modified `View` according to the condition.
    @FViewBuilder
    func `if`<Content: FBodyComponent, OtherContent: FBodyComponent>(_ condition: @autoclosure () -> Bool, transform: (Self) -> Content, else othertransform: ((Self) -> OtherContent)) -> FBody {
        if condition() {
            transform(self)
        } else {
            othertransform(self)
    ...
    @FViewBuilder
    func `if`<T, Content: FBodyComponent>(_ optional: T?, transform: (Self, T) -> Content) -> FBody {
        if let optional {
    ...
    /// - Returns: Either the original `View` or the modified `View` according to the optional value.
    @FViewBuilder
    func `if`<T, Content: FBodyComponent, OtherContent: FBodyComponent>(_ optional: T?, transform: (Self, T) -> Content, else othertransform: ((Self) -> OtherContent)) -> FBody {
        if let optional {
            transform(self, optional)
        } else {
            othertransform(self)
    ...
    /// - Returns: Either the original `View` or the modified `View` according to the condition.
    @FViewBuilder
    func `switch`<T: Hashable, Content: FBodyComponent>(_ condition: @autoclosure () -> T, cases: [T: (Self) -> FBodyComponent], default transform: ((Self) -> Content)? = nil) -> FBody {
    ...
    @FViewBuilder
    func wrapped<Content: FBodyComponent>(transform: (Self) -> Content) -> FBody {
        transform(self)
    }
}
